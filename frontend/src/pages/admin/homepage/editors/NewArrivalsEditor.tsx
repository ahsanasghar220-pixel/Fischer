...
interface NewArrivalsEditorProps {
  section: Section
  data: HomepageData
  open: boolean
  onClose: () => void
...
export default function NewArrivalsEditor({
  section,
  data,
  open,
  onClose,
  ...
  isSectionPending,
  isProductsPending,
}: NewArrivalsEditorProps) {
  const [title, setTitle] = useState(section.title || '')
  const [subtitle, setSubtitle] = useState(section.subtitle || '')
  const [source, setSource] = useState(section.settings?.source || 'manual')
  const [displayCount, setDisplayCount] = useState(section.settings?.display_count || 8)
  const [daysThreshold, setDaysThreshold] = useState(section.settings?.days_threshold || 30)
  const [selected, setSelected] = useState<number[]>([])

  useEffect(() => {
    const newArrivalProducts = data?.homepage_products
      ?.filter(hp => hp.section === 'new_arrivals')
      .map(hp => hp.product_id) || []
    setSelected(newArrivalProducts)
  ...
  const handleSaveAll = () => {
    ...
    onSaveProducts('new_arrivals', selected)
  }
  ...
  return (
    <EditorPanel
      open={open}
      onClose={onClose}
      title={SECTION_META.new_arrivals.label}
      description={SECTION_META.new_arrivals.editorDescription}
    ...
    >
      <div className="space-y-5">
        <FormField label="Section Title" tooltip="The heading for the New Arrivals section.">
          <input
            type="text"
            value={title}
        ...
        <FormField label="Section Subtitle" tooltip="Supporting text below the heading.">
          <textarea
            ...
            rows={2}
            placeholder="Check out our latest additions to the catalog"
            className="w-full px-4 py-2 border border-dark-200 dark:border-dark-600 rounded-lg bg-white dark:bg-dark-700 text-dark-900 dark:text-white"
          />
        </FormField>

        <div className="grid grid-cols-2 gap-4">
          <FormField label="Source" tooltip="Choose 'Manual' to hand-pick products, or 'Automatic' to show recently added products.">
            <select
              value={source}
            ...
            >
              ...
              <option value="auto">Automatic (recently added)</option>
            </select>
          </FormField>
          <FormField label="Display Count" tooltip="Maximum number of products to show." helpText="Default: 8">
            <input
              type="number"
              value={displayCount}
              onChange={e => setDisplayCount(parseInt(e.target.value) || 8)}
              min={1}
              max={20}
              className="w-full px-4 py-2 border border-dark-200 dark:border-dark-600 rounded-lg bg-white dark:bg-dark-700 text-dark-900 dark:text-white"
            />
        ...
        {source === 'auto' && (
          <FormField
            ...
            tooltip="Products added within this many days will be considered 'new arrivals' when using automatic mode."
            helpText="Default: 30 days"
          >
            <input
              type="number"
              value={daysThreshold}
        ...
        <div className="border-t border-dark-200 dark:border-dark-700 pt-5">
          ...
          <p className="text-xs text-dark-500 dark:text-dark-400 mb-3">
            {source === 'auto'
              ...
              : 'Select the products to display in the New Arrivals section. Use the arrows to reorder.'}
          </p>
